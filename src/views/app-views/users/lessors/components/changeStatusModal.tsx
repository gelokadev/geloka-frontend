...
const ChangeStatusModal: React.FC<IProps> = ({ visible, status, handleOk, handleCancel }) => {
    ...
    return (
        <Modal title={`${status ? 'Ré-activation' : 'Suspension'} du bailleur`} visible={visible} onOk={() => handleOk(reason)} onCancel={handleCancel}>
            <p>Vous allez {`${status ? 'ré-activer' : 'suspendre'}`} ce bailleur, quelle est la raison de cette action?</p>
